  ...
  private getServiceLabel(duration: number): string {
    switch (duration) {
      ...
      case 6:
        return '6 Hours WiFi';
      case 12:
        return '12 Hours WiFi';
      case 24:
        return '24 Hours WiFi';
      default:
        return `${duration} Hours WiFi`;
    }
  }

  public collectDataForPeriod(
    period: 'week' | 'month' | 'year'
  ): CollectedReportData {
    ...
    const totalRevenue = filteredTokens.reduce(
      (sum, token) => sum + token.price,
      0
    );
    const uniqueCustomers = new Set(
      filteredTokens.map(token => token.recipientPhone)
    ).size;
    const avgTransactionValue =
      filteredTokens.length > 0 ? totalRevenue / filteredTokens.length : 0;
    ...
    const payment_methods = Array.from(paymentMap.entries())
      .map(([method, amount]) => ({
        method,
        amount,
    ...
    return {
      ...
      avgTransactionValue,
      transactions_data: transactions_data.sort(
