...
const ReportsAnalytics = ({ language }: ReportsAnalyticsProps) => {
  ...
  const translations = {
    ...
    ar: {
      ...
      sampleDataDesc:
        'قم بإنشاء بعض رموز الواي فاي لرؤية البيانات الحقيقية في التقارير',
    ...
    try {
      ...
      if (!dataCollector.hasData()) {
        toast({
          ...
          description:
            'Please generate some WiFi tokens first to create reports.',
          variant: 'destructive',
        });
        return;
      }
    ...
    <div className="space-y-6">
        ...
        <CardContent className="pt-4">
          <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:gap-3">
            ...
            <div className="flex-1">
              <h4
                className={`font-medium ${hasRealData ? 'text-blue-800' : 'text-orange-800'}`}
              >
                {hasRealData ? t.realDataIndicator : t.sampleDataIndicator}
              </h4>
              <p
                className={`text-sm ${hasRealData ? 'text-blue-700' : 'text-orange-700'}`}
              >
                {hasRealData ? t.realDataDesc : t.sampleDataDesc}
              </p>
              {hasRealData && dataSummary && (
                <div className="text-xs text-blue-600 mt-1">
                  {dataSummary.totalTokens} tokens •{' '}
                  {dataSummary.totalRevenue.toLocaleString()} SSP total revenue
                </div>
              )}
            </div>
            <Badge
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select date range and generate comprehensive PDF reports
          </CardDescription>
        </CardHeader>
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div className="text-sm text-blue-700">
                <p>
                  ...
                  {customDateRange.startDate || 'Not selected'}
                </p>
                <p>
                  <strong>End Date:</strong>{' '}
                  {customDateRange.endDate || 'Not selected'}
                </p>
              </div>
            </div>
          ...
          <Card key={index}>
            ...
            <CardContent>
              ...
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                <span
                  className={kpi.isIncrease ? 'text-green-600' : 'text-red-600'}
                >
                  {kpi.change}
                </span>
